...
module.exports = {
  ...
  addToCart: async (req, res) => {
    ...
    const findInputBranch = await db.Stocks.findOne({
     ...
     const branchCart = findAvailableCart.dataValues.Product.dataValues.Stocks[0].dataValues.branch_id
    ...
    if( branchCart !== newBranchCart){
        ...
        if (!findUserCart) {
            ...
            res.status(200).send({
                message: `Amount of this product in your cart is now ${currentQty + 1}`,
                data: addProductCart
            });
        }
    }   
...
reduceCartOne: async (req, res) => {
    try{
        ...
        if(updatedQty === 0){
            ...
            res.status(200).send({
                message: `Amount of this product in your cart is now reduce to ${currentQty - 1}`,
                data: addProductCart
            });
        }     
    }
